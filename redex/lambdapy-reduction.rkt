...
(define λπ-red
  (reduction-relation
   ...
   (--> ((in-hole E (get-field (pointer-val ref_obj) string)) ε Σ)
        ...
        (where (obj-val (pointer-val ref) mval ((string_1 ref_2) ...)) (store-lookup Σ ref_obj))
        (where (Σ_result val_result) (class-lookup (pointer-val ref_obj) (store-lookup Σ ref) string Σ))
